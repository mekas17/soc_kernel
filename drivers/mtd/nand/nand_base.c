...
#ifdef CONFIG_MTD_NAND_LX28XX
		pr_info("MTD_NAND_LX28XX\n");
		u32 tmp;
		tmp = chip->read_long(mtd);
		maf_id = (u8)tmp;
		dev_id = (u8)(tmp >> 8);
		pr_info("maf_id = 0x%x, dev_id = 0x%x\n", maf_id, dev_id);
#else
...
#ifdef CONFIG_MTD_NAND_LX28XX
	for (i = 0; i < ARRAY_SIZE(chip->id.data)/4; i++) {
		int temp = chip->read_long(mtd);
		id_data[i*4] = (u8)temp;
		id_data[i*4 + 1] = (u8)(temp >> 8);
		id_data[i*4 + 2] = (u8)(temp >> 16);
		id_data[i*4 + 3] = (u8)(temp >> 24);
	}
	pr_info("id : %02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
